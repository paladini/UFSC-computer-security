...
# p/ 32 bytes de chave:
#   - cria 60 palavras/240 bytes durante a expansao.
#   - Tamanho da chave por round: 4 palavras / 16 bytes
#   - 14 rodadas
def key_expansion(chave):

    chave = bytearray.fromhex(chave)
    print(chave)
...
class AES(object):
    ...
    def __init__(self, chave):
        self.expandir_chave(chave)

    def expandir_chave(chave):
        self.round_keys = key_expansion(chave)
...
#         for rk in self.round_keys[0]:
#             print(str(rk) + " - size of {} bytes.".format(sys.getsizeof(rk)))
